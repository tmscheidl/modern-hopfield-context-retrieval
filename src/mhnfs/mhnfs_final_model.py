...
class MHNfsFinalModel(nn.Module):
    ...
    def __init__(self, cross_attention: nn.Module,
                 ...
                 prediction_scaling: float = 0.044194173824159216):
        super().__init__()
    ...
    def forward(self, query, support_actives, support_inactives,
        ...
        query, actives, inactives = self.cross_attention(
            query, support_actives, support_inactives, mask_actives, mask_inactives
        )
        ...
        sim_active = self.similarity_module(
            query_embedding=query, support_set_embeddings=actives,
            padding_mask=mask_actives, support_set_size=support_size_a)
        sim_inactive = self.similarity_module(
            query_embedding=query, support_set_embeddings=inactives,
            padding_mask=mask_inactives, support_set_size=support_size_i)
